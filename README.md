...
# 简介
与目前流行的以路由拦截实现微前端渲染不同是，Micro App不再以监听路由的变化来渲染微前端，而是借鉴了WebComponent的思想，通过CustomElement结合自定义的ShadowDom，将所有功能都封装到一个类WebComponent组件中，从而实现微前端的组件化渲染，并且由于自定义ShadowDom的元素隔离特性，Micro App不需要子应用在入口文件暴露出方法及修改webpack配置，这是目前接入成本最低的微前端框架。
