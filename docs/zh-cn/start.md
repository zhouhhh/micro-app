...
> 1、`name`和`url`属性必传
>
> 2、每个`name`都对应一个应用，不可以带有 `.`、`#` 等特殊符号。
>
> 3、`url`属性和子应用路由的关系请查看[这里](/zh-cn/route)
>
> 4、`baseurl`是基座应用分配给子应用的路由前缀，非必传，默认值为空字符串。
> 
> 5、子应用必须支持跨域访问，跨域配置参考[这里](/zh-cn/questions?id=_2、子应用静态资源一定要支持跨域吗？)
